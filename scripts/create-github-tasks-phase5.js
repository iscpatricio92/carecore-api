...
/**
 * Script para crear issues de GitHub desde el archivo de tareas de la Fase 5
 ...
 *
 * Uso:
 *   node scripts/create-github-tasks-phase5.js
 *   node scripts/create-github-tasks-phase5.js --dry-run  # Solo muestra lo que haría
 */
...
function parseTasksFile() {
  const tasksFile = path.join(__dirname, '../docs/tasks/PHASE5_TESTING_AND_DOCUMENTATION.md');
  ...
  // El regex captura todo hasta el salto de línea después de Labels
  const taskRegex =
    /### Tarea (\d+): (.+?)\n\n\*\*Título:\*\* `(.+?)`\n\n\*\*Descripción:\*\*\n```markdown\n([\s\S]+?)```\n\n\*\*Labels:\*\* `(.+?)`\n/g;

  let match;
  while ((match = taskRegex.exec(content)) !== null) {
    const [, taskNum, taskName, title, description, labelsStr] = match;

    // Parsear labels: el formato capturado es "label1`, `label2`, `label3"
    // Usar split por '`, `' y limpiar backticks restantes
...
Ver [PHASE5_TESTING_AND_DOCUMENTATION.md](../docs/tasks/PHASE5_TESTING_AND_DOCUMENTATION.md) para tareas detalladas.
Ver [AUTH_IMPLEMENTATION_PLAN.md](../docs/AUTH_IMPLEMENTATION_PLAN.md) para contexto completo.`;
  ...
  const tmpFile = path.join(__dirname, '..', `.tmp-hu-phase5-${Date.now()}.md`);
  ...
  try {
    ...
    const command = [
      ...
      `--body-file "${tmpFile}"`,
      `--label "enhancement,test,documentation,phase-5,HU"`,
      `--repo "${OWNER}/${REPO}"`,
    ].join(' ');
...
function createIssue(task, parentIssueNumber = null) {
  ...
  let fullBody = `${body}\n\n---\n\n**Tarea ${task.number} de la Fase 5: Testing y Documentación**\n\nVer [PHASE5_TESTING_AND_DOCUMENTATION.md](../docs/tasks/PHASE5_TESTING_AND_DOCUMENTATION.md) para contexto completo.`;
  ...
  if (DRY_RUN) {
    ...
    log(`   Labels: ${task.labels.join(', ')}`, 'blue');
  ...
  const tmpFile = path.join(tmpDir, `.tmp-issue-phase5-${task.number}-${Date.now()}.md`);
...
function ensureRequiredLabels() {
  ...
  const requiredLabels = [
    ...
    { name: 'documentation', color: 'c5def5', description: 'Documentation related' },
    { name: 'auth', color: 'd73a4a', description: 'Authentication and authorization related' },
    { name: 'phase-5', color: '7057ff', description: 'Fase 5: Testing y Documentación' },
    { name: 'unit-test', color: '0e8a16', description: 'Unit testing related' },
    { name: 'e2e-test', color: '1d76db', description: 'E2E testing related' },
    { name: 'oauth2', color: '0052cc', description: 'OAuth2 related' },
    { name: 'verification', color: 'fbca04', description: 'Practitioner verification related' },
    { name: 'smart-fhir', color: '5319e7', description: 'SMART on FHIR related' },
    { name: 'keycloak', color: 'b60205', description: 'Keycloak related' },
    { name: 'roles', color: 'c2e0c6', description: 'Roles and permissions related' },
    { name: 'scopes', color: 'bfd4f2', description: 'OAuth2 scopes related' },
    { name: 'integration', color: '0052cc', description: 'Integration with external services' },
    { name: 'readme', color: 'c5def5', description: 'README updates' },
    { name: 'developer-guide', color: 'c5def5', description: 'Developer guide related' },
  ];
...
// Main
async function main() {
  log('🚀 Script de creación de issues para Fase 5: Testing y Documentación\n', 'bright');
