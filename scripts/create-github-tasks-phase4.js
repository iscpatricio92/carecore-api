...
/**
 * Script para crear issues de GitHub desde el archivo de tareas de la Fase 4
 ...
 *
 * Uso:
 *   node scripts/create-github-tasks-phase4.js
 *   node scripts/create-github-tasks-phase4.js --dry-run  # Solo muestra lo que haría
 */
...
function parseTasksFile() {
  const tasksFile = path.join(__dirname, '../docs/tasks/PHASE4_SMART_ON_FHIR.md');
...
Ver [PHASE4_SMART_ON_FHIR.md](../docs/tasks/PHASE4_SMART_ON_FHIR.md) para tareas detalladas.
Ver [AUTH_IMPLEMENTATION_PLAN.md](../docs/AUTH_IMPLEMENTATION_PLAN.md) para contexto completo.`;
  ...
  const tmpFile = path.join(__dirname, '..', `.tmp-hu-phase4-${Date.now()}.md`);
  ...
  try {
    ...
    const command = [
      ...
      `--body-file "${tmpFile}"`,
      `--label "enhancement,auth,phase-4,HU"`,
      `--repo "${OWNER}/${REPO}"`,
    ].join(' ');
...
function createIssue(task, parentIssueNumber = null) {
  ...
  let fullBody = `${body}\n\n---\n\n**Tarea ${task.number} de la Fase 4: SMART on FHIR**\n\nVer [PHASE4_SMART_ON_FHIR.md](../docs/tasks/PHASE4_SMART_ON_FHIR.md) para contexto completo.`;
  ...
  const tmpFile = path.join(tmpDir, `.tmp-issue-phase4-${task.number}-${Date.now()}.md`);
...
function ensureRequiredLabels() {
  ...
  const requiredLabels = [
    ...
    { name: 'phase-4', color: '7057ff', description: 'Fase 4: SMART on FHIR' },
    ...
    { name: 'audit', color: 'fbca04', description: 'Audit logging related' },
  ];
...
// Main
async function main() {
  log('🚀 Script de creación de issues para Fase 4: SMART on FHIR\n', 'bright');
