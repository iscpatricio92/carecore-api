...
/**
 * Script para crear issues de GitHub desde el archivo de tareas de la Fase 2
 *
 * Requisitos:
 ...
 *
 * Uso:
 *   node scripts/create-github-tasks-phase2.js
 *   node scripts/create-github-tasks-phase2.js --dry-run  # Solo muestra lo que haría
 */
...
function parseTasksFile() {
  const tasksFile = path.join(__dirname, '../docs/tasks/PHASE2_NESTJS_INTEGRATION.md');
  ...
  // Buscar cada sección de tarea (incluye Tarea 0)
  const taskRegex =
    /### Tarea (\d+): (.+?)\n\n\*\*Título:\*\* `(.+?)`\n\n\*\*Descripción:\*\*\n```markdown\n([\s\S]+?)```\n\n\*\*Labels:\*\* `(.+?)`/g;
...
function createIssue(task, milestone = null) {
  ...
  const tmpFile = path.join(tmpDir, `.tmp-issue-phase2-${task.number}-${Date.now()}.md`);
...
// Main
async function main() {
  log('🚀 Script de creación de issues para Fase 2: Integración con NestJS\n', 'bright');
