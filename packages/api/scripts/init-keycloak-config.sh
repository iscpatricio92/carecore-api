...
# Método robusto que maneja valores con espacios y caracteres especiales
if [ -f "$PROJECT_ROOT/.env.local" ]; then
  # Usar un método que parsea línea por línea correctamente
  # Esto evita problemas con valores que contienen espacios o caracteres especiales
  while IFS= read -r line || [ -n "$line" ]; do
    # Ignorar comentarios y líneas vacías
    [[ "$line" =~ ^[[:space:]]*# ]] && continue
    [[ -z "${line// }" ]] && continue
    # Verificar que sea una asignación válida (VAR=value)
    if [[ "$line" =~ ^[[:space:]]*[A-Za-z_][A-Za-z0-9_]*= ]]; then
      # Extraer nombre y valor usando IFS temporalmente
      old_ifs="$IFS"
      IFS='=' read -r var_name var_value <<< "$line"
      IFS="$old_ifs"
      # Eliminar espacios del nombre de la variable
      var_name="${var_name// /}"
      # Exportar la variable con su valor
...
if [ "$REALM_OK" = true ] && [ "$ROLES_OK" = true ] && [ "$CLIENTS_OK" = true ] && [ "$SCOPES_OK" = false ]; then
  ...
  echo ""
  if [ -f "$PROJECT_ROOT/packages/api/keycloak/init/create-scopes.sh" ]; then
    bash "$PROJECT_ROOT/packages/api/keycloak/init/create-scopes.sh" "$ACCESS_TOKEN" || {
...
if [ "$REALM_OK" = false ] || [ "$ROLES_OK" = false ] || [ "$CLIENTS_OK" = false ]; then
  ...
  if [ -f "$PROJECT_ROOT/packages/api/keycloak/init/setup-keycloak.sh" ]; then
    # Ejecutar de forma no interactiva (no recrear realm si existe)
    echo "N" | bash "$PROJECT_ROOT/packages/api/keycloak/init/setup-keycloak.sh" || {
