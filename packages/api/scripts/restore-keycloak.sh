...
# Obtener directorio del proyecto (root del monorepo)
...
# Archivos de backup
REALM_BACKUP="packages/api/keycloak/backups/realms/carecore-realm-${TIMESTAMP}.json"
DB_BACKUP="packages/api/keycloak/backups/db/keycloak-db-${TIMESTAMP}.dump"
...
if [ ! -f "${REALM_BACKUP}" ]; then
  ...
  ls -1 packages/api/keycloak/backups/realms/ 2>/dev/null || echo "  (ninguno)"
  exit 1
fi
...
if [ ! -f "${DB_BACKUP}" ]; then
  ...
  echo ""
  echo "Backups disponibles:"
  ls -1 packages/api/keycloak/backups/db/ 2>/dev/null || echo "  (ninguno)"
  exit 1
fi
